...
func TestMigrate_BootstrapsLegacyDBWithoutIntakeColumns(t *testing.T) {
	...
	if maxV != 19 {
		t.Fatalf("want version 19 after bootstrap; got %d", maxV)
	}
}
...
func TestMigrate_BootstrapPreservesWorktreeAndSeedsAllVersions(t *testing.T) {
	...
	if rows != 19 {
		t.Errorf("migration_versions row count = %d, want 19 (bootstrap missed markers)", rows)
	}
}
...
func TestMigrate_IntakeInterviewIdempotent_From008(t *testing.T) {
	...
	if maxV != 19 {
		t.Fatalf("want max version 19 after 008→019 upgrade; got %d", maxV)
	}
}
